...
soap_rs_dvr_features = representation.transform(
    frames).get_features_by_species(representation)[(1,)][::5]
representation = SphericalInvariants(**hypers_rs_gto)
soap_rs_gto_features = representation.transform(
    frames).get_features_by_species(representation)[(1,)][::5]
representation = SphericalInvariants(**hypers_ps_dvr)
soap_ps_dvr_features = representation.transform(
    frames).get_features_by_species(representation)[(1, 1)][::5]
representation = SphericalInvariants(**hypers_ps_gto)
soap_ps_gto_features = representation.transform(
    frames).get_features_by_species(representation)[(1, 1)][::5]

feature_spaces = [soap_rs_dvr_features, soap_rs_gto_features,
                  soap_ps_dvr_features, soap_ps_gto_features]
...
def plot_feature_space_measure_matrix(measure_matrix):
    ...
    plt.yticks(np.arange(4), [r'$\textrm{RS}_{\textrm{DVR}}$', r'$\textrm{RS}_{\textrm{GTO}}$',
                              r'$\textrm{PS}_{\textrm{DVR}}$', r'$\textrm{PS}_{\textrm{GTO}}$'], fontsize=19)
    plt.xticks(np.arange(4), [r'$\textrm{RS}_{\textrm{DVR}}$', r'$\textrm{RS}_{\textrm{GTO}}$',
                              r'$\textrm{PS}_{\textrm{DVR}}$', r'$\textrm{PS}_{\textrm{GTO}}$'], fontsize=19)
    plt.ylim(-0.5, 3.5)
    plt.title(
        "Distortion $\|\\tilde{X}_{F'}-X_F Q\|/\|\\tilde{X}_{F'}'\|$", fontsize=22)
