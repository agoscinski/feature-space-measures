...
def gfr_pairwise_experiment(
    ...
    FRE_matrix, FRD_matrix = compute_feature_space_reconstruction_measures(
        two_split, splitting_ratio, seed, noise_removal, regularizer, feature_spaces1, feature_spaces2
    )
...
def gfr_all_pairs_experiment(
    dataset_name, nb_samples, features_hypers, two_split, splitting_ratio, seed, noise_removal, regularizer
):
    metadata, experiment_id = store_metadata(
        dataset_name, nb_samples, features_hypers, two_split, splitting_ratio, seed, noise_removal, regularizer
    )
    ...
    FRE_matrix, FRD_matrix = compute_feature_space_reconstruction_measures(
        two_split, splitting_ratio, seed, noise_removal, regularizer, feature_spaces
    )
...
def lfre_all_pairs_experiment(
    dataset_name, nb_samples, features_hypers, nb_local_envs, two_split, splitting_ratio, seed, regularizer
):
    metadata, experiment_id = store_metadata(
        dataset_name, nb_samples, features_hypers, two_split, splitting_ratio, seed, noise_removal=False, regularizer=regularizer, nb_local_envs=nb_local_envs
    )
    frames = read_dataset(dataset_name, nb_samples)
    feature_spaces = compute_representations(features_hypers, frames)
...
def ghfre_experiment(
    dataset_name, nb_samples, features_hypers, two_split, splitting_ratio, seed, regularizer, hidden_feature_name,
):
    metadata, experiment_id = store_metadata(
        dataset_name, nb_samples, features_hypers, two_split, splitting_ratio, seed, noise_removal=False, regularizer=regularizer, hidden_feature_name=hidden_feature_name)
    frames = read_dataset(dataset_name, nb_samples)
    ...
    FRE_matrix, FRD_matrix = compute_feature_space_reconstruction_measures(
        two_split, splitting_ratio, seed, False, regularizer, feature_spaces, [hidden_feature for _ in range(len(feature_spaces))]
    )
...
def hfre_experiment(
    ...
    FRE_vector = feature_spaces_hidden_feature_reconstruction_errors(
        feature_spaces, hidden_feature, two_split, splitting_ratio, seed, regularizer)
...
def store_metadata(
    dataset_name, nb_samples, features_hypers, two_split, splitting_ratio, seed, noise_removal, regularizer, hidden_feature_name = None, nb_local_envs = None
):
    metadata = {
        # Methane
...
def compute_feature_space_reconstruction_measures(
    two_split, splitting_ratio, seed, noise_removal, regularizer, feature_spaces1, feature_spaces2=None 
):
    ...
    if two_split:
        if feature_spaces2 is None:
            FRE_matrix, FRD_matrix = two_split_reconstruction_measure_all_pairs(
                feature_spaces1, splitting_ratio=splitting_ratio, seed=seed, noise_removal=noise_removal, regularizer=regularizer
            )
        else:
            FRE_matrix, FRD_matrix = two_split_reconstruction_measure_pairwise(
                feature_spaces1, feature_spaces2, splitting_ratio=splitting_ratio, seed=seed, noise_removal=noise_removal, regularizer=regularizer
            )
    else:
        if feature_spaces2 is None:
