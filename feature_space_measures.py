...
def feature_space_reconstruction_measures(features1, features2, svd_method='gesdd'):
    ...
    U2,S2,V2 = scipy.linalg.svd(features1_U.T.dot(reconstructed_features2_VT), lapack_driver=svd_method)
    Q = U2.dot(V2)
    # see paper for derivation of alpha
    alpha = np.trace(features1_U.dot(Q).T.dot(reconstructed_features2_VT))/np.trace(features1_U.dot(features1_U.T))
    FRD = np.linalg.norm(alpha*features1_U.dot(Q)-reconstructed_features2_VT)/np.linalg.norm(reconstructed_features2_VT)
    return FRE, FRD

def reconstruction_measure_matrix(feature_spaces):
    # feature_spaces: a list of feature spaces [X_{H_1}, ..., X_{H_n}]
    # return: a matrix containing the FRE(X_{H_i},X_{H_j})
    #         a matrix containing the FRD(X_{H_i},X_{H_j})
    ...
    for i in range(len(feature_spaces)):
        for j in range(len(feature_spaces)):
            FRE_matrix[i,j], FRD_matrix[i,j] = feature_space_reconstruction_measures(feature_spaces[i],feature_spaces[j])
...
def distance2_from_features(features):
    distmat = np.sum(features**2,axis=1)[:,np.newaxis] + np.sum(features**2,axis=1)[np.newaxis,:] -2*features.dot(features.T)
    distmat[distmat<0] = 0
    return distmat
...
def features_from_distance2(distance):
    D, U = np.linalg.eigh(-(np.eye(len(distance))-np.ones((len(distance),len(distance)))/len(distance)).dot(distance).dot(np.eye(len(distance))-np.ones((len(distance),len(distance)))/len(distance))/2)
    if np.min(D)<0:
        #print('negative eigenvalue encounterd',np.min(D))
        D[D<0]=0
    return np.flip(U,axis=1).dot(np.diag(np.sqrt(np.flip(D))))
...
def local_truncated_reconstruction(features1, features2, nb_of_local_env = 10):
    ...
    for local_env in range(len(features1)):
        ...
        truncated_features1 = features_from_kernel(centered_kernel_from_distance2(distance2_from_features(features1[local_env_idx])))
        truncated_features2 = features_from_kernel(centered_kernel_from_distance2(distance2_from_features(features2[local_env_idx])))
        reconstruction_error[local_env] = feature2_reconstruction_error_vector(truncated_features1, truncated_features2)[0]
...
def global_truncated_reconstruction(features1, features2):
    distance2 = distance2_from_features(features1)
    truncated_features1 = features_from_kernel(centered_kernel_from_distance2(distance2_from_features(features1)))
    truncated_features2 = features_from_kernel(centered_kernel_from_distance2(distance2_from_features(features2)))
