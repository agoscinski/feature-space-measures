...
def plot_feature_space_measure_matrix(measure_matrix, title, name, metadata):
    ...
    ticks = [
        ticks_table[(
            metadata["features_hypers"][i]["soap_type"],
...
try:
    ...
    FRE_matrix = np.load(RESULTS_FOLDER + "gfre_mat-" + hash_value + ".npy")
    FRD_matrix = np.load(RESULTS_FOLDER + "gfrd_mat-" + hash_value + ".npy")
    with open(f"{RESULTS_FOLDER}metadata-{hash_value}.json", "r") as metadata_file:
        metadata = json.load(metadata_file)
    plot_feature_space_measure_matrix(FRE_matrix, r"Error $\|X_{F'}-X_F P\|/\|\\tilde{X}_{F'}'\|$", "fre_mat-" + hash_value, metadata)
    plot_feature_space_measure_matrix(FRD_matrix, r"Distortion $\|\tilde{X}_{F'}-X_F Q\|/\|\tilde{X}_{F'}'\|$", "frd_mat-" + hash_value, metadata)
except IndexError:
