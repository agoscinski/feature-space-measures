...
def plot_radial_angular_trajectories(gto_measure_matrix, dvr_measure_matrix, title, name, gto_metadata, dvr_metadata):
    ...
    ax1.plot(max_angulars, gto_measure_matrix[0], label="GTO")
    ax1.plot(max_angulars, dvr_measure_matrix[0], label="DVR")
    #plt.plot(max_angulars, gto_measure_matrix[np.tril_indices(len(max_angulars),-1)], label="DVR")
    # add x axis for max radial
    ax1.set_xlabel("max angular $l$", fontsize=18)
    ax1.set_xticks(max_angulars[:-1])
    ax1.set_xticklabels(max_angulars[:-1], fontsize=18)

    #ax1.set_ylabel(title, fontsize=18)
    #locs, labels = plt.yticks()
    #ax1.set_yticks(locs)
    #ax1.set_yticklabels(locs, fontsize=18)
    #ax1.tick_params(labelsize=15)

    ax2.set_xlabel("max radial $n$", fontsize=18)
    ax2.set_xticks(max_radials[:-1])
    ax2.set_xticklabels(max_radials[:-1], fontsize=18)
...
plot_radial_angular_trajectories(gto_gfre_matrix, dvr_gfre_matrix, "$\\textrm{GRFE}((n+2,l+1),(n,l))$", "gfre_radial_angular-gto-"+gto_hash_value+"-dvr-"+dvr_hash_value, gto_metadata, dvr_metadata)
plot_radial_angular_trajectories(gto_gfrd_matrix, dvr_gfrd_matrix, "$\\textrm{GRFD}((n+2,l+1),(n,l))$", "gfrd_radial_angular-gto-"+gto_hash_value+"-dvr-"+dvr_hash_value, gto_metadata, dvr_metadata)
